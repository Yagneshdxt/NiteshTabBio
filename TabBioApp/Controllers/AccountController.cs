...
namespace TabBioApp.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        private SendMails _sendMails = new SendMails();
        ...
        public IHttpActionResult Logout()
        {
            ...
            //String authorization = HttpContext.Current.Request.Headers.GetValues("Authorization").ToString();
        ...
        public IHttpActionResult ResetPassword(string code, string emailId, string newPassword)
        {
            if (String.IsNullOrEmpty(code))
            {
                ModelState.AddModelError("CriticalError", "Forgot password link expired");
                return BadRequest(ModelState);
            }

            if (String.IsNullOrEmpty(newPassword))
            {
                ModelState.AddModelError("PasswordRequired", "Password Cannot be empty");
                return BadRequest(ModelState);
            }

            if (String.IsNullOrEmpty(emailId))
            {
                ModelState.AddModelError("EmailNotFound", "Entered EmailId not Found");
                return BadRequest(ModelState);
            }

            var userByEmailId = UserManager.FindByEmail(emailId);

            if (userByEmailId == null)
            {
                if (!ModelState.Any(a => a.Key == "EmailNotFound"))
                {
                    ModelState.AddModelError("EmailNotFound", "Entered EmailId not Found");
                }
                return BadRequest(ModelState);
            ...
            IdentityResult result = UserManager.ResetPassword(userByEmailId.Id, code, newPassword);
        ...
        [HttpGet, AllowAnonymous, Route("GetForgotPassword")]
        public IHttpActionResult GetForgotPassword(string emailId)
        {
            ...
            string passwordResetToken = UserManager.GeneratePasswordResetToken(userByEmailId.Id);
            ...
            callbackUrl = this.Url.Link("Default", new { Controller = "Account", Action = "ResetPassword", code = passwordResetToken });
            _sendMails.SendEmail(userByEmailId.Email, "Password Reset link", "Please reset your password by clicking <a href=\"" + callbackUrl + "\">here</a>", true, null);

            return Ok("Email Forwared to your registered email ID Click on the link in email to reset your password.");
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var user = new ApplicationUser() { UserName = model.Email, Email = model.Email, FirstName = model.FirstName, LastName = model.LastName, UserUniqueCode = uniqueCode };
